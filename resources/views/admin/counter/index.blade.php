...
@section('content')
                        ...
                        <div class="card">
                            <div class="card-header d-flex justify-content-between align-items-center">
                                ...
                                <nav class="navbar navbar-light">
                                    ...
                                    <a href="{{ route('admin-counter-add') }}" class="btn btn-primary my-2 my-sm-0 ms-1">
                                        Add</a>
                                </nav>
                            </div>
                            <div class="card-body">
                                @if ($counters->count() > 0)
                                                ...
                                                @foreach ($counters as $counter)
                                                    <tr id="tr{{ $counter->id }}">
                                                        ...
                                                        <td> <img
                                                                ...
                                                                height="30"></td>
                                                        <td>{{ $counter->title }}</td>
                                                        <td>{{ $counter->caption }}</td>
                                                        <td style="text-align:center">{{ $counter->order }}</td>
                                                        <td style="text-align:center"><a
                                                                href="{{ route('admin-counter-add', ['id=' . $counter->id]) }}"
                                                                class="btn btn-sm btn-warning rounded-pill" title="Edit"
                                                                data-bs-toggle="tooltip" data-bs-placement="top"><i
                                                                    class="fas fa-pen"></i></a>
                                                            {{-- <button type="button"
                                                                class="btn btn-sm btn-danger rounded-pill delete-row-btn"
                                                                data-id="{{ $counter->id }}" title="Delete"
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-md-8 col-sm-6 col-xs-12">
                                            <div class="float-end"> {{ $counters->links('pagination::bootstrap-4') }}
                                            </div>
                                        </div>
                                    </div>
                                @else
    ...
    @section('footer-scripts')
        @include('admin.counter.js.index')
    @endsection
